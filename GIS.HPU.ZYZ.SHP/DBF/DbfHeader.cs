...
namespace GIS.HPU.ZYZ.SHP.DBF
{
    ...
    public class DbfHeader : ICloneable
    {
        ...
        /// <param name="sName"></param>
        /// <returns></returns>
        public int FindColumnEx(string sName) 
        {
            if (mColumnNameIndex == null)
            {
                mColumnNameIndex = new Dictionary<string, int>(mFields.Count);
            ...
            //if (mColumnNameIndex.TryGetValue(sName, out columnIndex))
            //    return columnIndex;
            foreach (var item in mColumnNameIndex) {
                if (item.Key.Contains(sName.Substring(0, sName.Length - 1))) {
        ...
        public void Write(BinaryWriter writer)
        {
            ...
            mUpdateDate = DateTime.Now;
            //Update date format is YYMMDD, which is different from the column Date type (YYYYDDMM) 从1990算起
            ...
            foreach (DbfColumn field in mFields)
            {
                ...
                char typeChar = (char)field.ColumnTypeChar;
                byte typeByte = Convert.ToByte(typeChar);
                writer.Write(typeChar);
        ...
        public void Read(BinaryReader reader)
        {
            ...
            if (nFileType != 0x03)//默认的是3 国家系统导出的是3 
            ...
            {
                // read the field name				
                //char[] buffer = new char[11];//有中文字符 不能这样写 一个中文字符占2char
                //buffer = reader.ReadChars(11);
                byte[] nbytes = reader.ReadBytes(11);
                string sFieldName = encoding.GetString(nbytes); //new string(nbytes);//不指定编码？
